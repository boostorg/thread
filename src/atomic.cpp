...
namespace boost {
	...
	{
		return InterlockedIncrement(const_cast<long*>(&x.value));
	}

	atomic_t::value_type decrement(atomic_t& x)
	{
		return InterlockedDecrement(const_cast<long*>(&x.value));
	}

	atomic_t::value_type swap(atomic_t& x, atomic_t::value_type y)
	{
		return InterlockedExchange(const_cast<long*>(&x.value), y);
	}

	atomic_t::value_type compare_swap(atomic_t& x, atomic_t::value_type y, atomic_t::value_type z)
	{
		return InterlockedCompareExchange(const_cast<long*>(&x.value), y, z);
	}
#else
	atomic_t::value_type increment(atomic_t& x)
	{
