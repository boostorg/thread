    ...
    public:
        ...
        bool timed_lock_shared(boost::system_time const& wait_until)
        {
            for(;;)
            {
                ...
                unsigned long const res=winapi::WaitForSingleObjectEx(semaphores[unlock_sem],::boost::detail::get_milliseconds_until(wait_until), 0);
        ...
        bool try_lock_shared_until(const chrono::time_point<chrono::system_clock, chrono::system_clock::duration>& tp)
        {
          for(;;)
          {
            ...
            if (tp>n) {
              ...
              res=winapi::WaitForSingleObjectEx(semaphores[unlock_sem],
        ...
        bool timed_lock(boost::system_time const& wait_until)
        {
            for(;;)
            {
                ...
                #endif
                unsigned long const wait_res=winapi::WaitForMultipleObjectsEx(2,semaphores,wait_all,::boost::detail::get_milliseconds_until(wait_until), 0);
        ...
        bool try_lock_until(const chrono::time_point<chrono::system_clock, chrono::system_clock::duration>& tp)
        {
          for(;;)
          {
            ...
            if (tp>n) {
              ...
              wait_res=winapi::WaitForMultipleObjectsEx(2,semaphores,wait_all,
                  static_cast<unsigned long>(rel_time.count()), 0);
            } else {
        ...
        void lock_upgrade()
        {
            for(;;)
            {
                ...
                BOOST_VERIFY(!winapi::WaitForSingleObjectEx(semaphores[unlock_sem],winapi::infinite, 0));
            }
        }
        ...
        void unlock_upgrade_and_lock()
        {
            ...
            for(;;)
            {
                ...
                if(current_state==old_state)
                {
                    if(!last_reader)
                    {
                        BOOST_VERIFY(!winapi::WaitForSingleObjectEx(upgrade_sem,detail::win32::infinite, 0));
                    }
                    break;
                }
                old_state=current_state;
