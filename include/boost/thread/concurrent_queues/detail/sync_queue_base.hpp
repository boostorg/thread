...
namespace detail
{
  ...
  protected:
    ...
    inline void notify_one(unique_lock<mutex>& )
    {
      not_empty_.notify_one();
    }

    inline void notify_all(unique_lock<mutex>& )
    {
  ...
  {
    if (! not_empty_.wait_until(lk, tp, boost::bind(&sync_queue_base<ValueType, Queue>::closed, boost::ref(*this), boost::ref(lk))))
      return queue_op_status::timeout;
    return queue_op_status::closed;
  }
