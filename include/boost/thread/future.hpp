...
{
  template <class T>
  csbl::shared_ptr<T> static_shared_from_this(T* that)
  {
    return csbl::static_pointer_cast<T>(that->shared_from_this());
  }
  template <class T>
  csbl::shared_ptr<T const> static_shared_from_this(T const* that)
  {
    return csbl::static_pointer_cast<T const>(that->shared_from_this());
  }
  ...
  using executors::executor;
    ...
    namespace detail
    {
        ...
        struct shared_state_base : csbl::enable_shared_from_this<shared_state_base>
        {
            ...
            typedef csbl::shared_ptr<shared_state_base> continuation_ptr_type;
            ...
            void set_executor_policy(executor_ptr_type aex, boost::lock_guard<boost::mutex> lk)
            {
              set_executor_policy(ex);
            }
...
#else
              typedef csbl::unique_ptr<T> storage_type;
#endif
        ...
        struct future_async_shared_state: future_async_shared_state_base<Rp>
        {
          ...
          static void run(csbl::shared_ptr<future_async_shared_state> that, BOOST_THREAD_FWD_REF(Fp) f)
          {
            try
            {
        ...
        private:
            ...
            struct registered_waiter
            {
                csbl::shared_ptr<detail::shared_state_base> future_;
                detail::shared_state_base::notify_when_ready_handle handle;
                count_type index;

                registered_waiter(csbl::shared_ptr<detail::shared_state_base> const& a_future,
...
//    }
    ...
    namespace detail
    {
      ...
      public:
        ...
        typedef csbl::shared_ptr<detail::shared_state<R> > future_ptr;
    ...
    namespace detail
    {
        ...
        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_executor_continuation_shared_state(executor_ptr_type& ex, boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_executor_continuation_shared_state(executor_ptr_type& ex, boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
    ...
    private:
        ...
        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_continuation_shared_state(executor_ptr_type& ex, boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_executor_continuation_shared_state(executor_ptr_type& ex, boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
...
#include <boost/detail/atomic_undef_macros.hpp>
          if(!atomic_load(&future_))
            {
                future_ptr blank;
                atomic_compare_exchange_strong(&future_,&blank,future_ptr(new detail::shared_state<R>));
            }
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    class promise<void>
    {
        typedef csbl::shared_ptr<detail::shared_state<void> > future_ptr;
        ...
        void lazy_init()
        {
            ...
            {
                future_ptr blank;
                atomic_compare_exchange_strong(&future_,&blank,future_ptr(new detail::shared_state<void>));
            }
#endif
        }
    public:
...
namespace boost
{
    ...
    {
      typedef csbl::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task_ptr;
      csbl::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef csbl::shared_ptr<detail::task_base_shared_state<R()> > task_ptr;
      csbl::shared_ptr<detail::task_base_shared_state<R()> > task;
  #endif
#else
    template<typename R>
    class packaged_task
...
namespace detail
{
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_deferred_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    csbl::shared_ptr<future_deferred_shared_state<Rp, Fp> >
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_async_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    csbl::shared_ptr<future_async_shared_state<Rp, Fp> >
...
namespace detail {
    ...
    {
      typedef csbl::shared_ptr<shared_state_base > storage_type;
      storage_type that;
      Fp f_;
    public:
...
#endif
      void operator()() {
        csbl::shared_ptr<shared_state<Rp> > that_ = csbl::static_pointer_cast<shared_state<Rp> >(that);
        if (! that) return;
        try {
...
#endif
      void operator()() {
        csbl::shared_ptr<shared_state<void> > that_ = csbl::static_pointer_cast<shared_state<void> >(that);
        if (! that) return;
        try {
          f_();
...
namespace detail {
    ...
    public:
      ...
      void init(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f)
      {
        ...
        typedef typename decay<Executor>::type Ex;
    ...
    BOOST_THREAD_FUTURE<Rp>
    make_future_executor_shared_state(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f) {
      csbl::shared_ptr<future_executor_shared_state<Rp> >
...
namespace detail
{
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      boost::lock_guard<boost::mutex> lk(this->mutex);
      this->thr_ = thread(&base_type::run, this->shared_from_this());
#else
      thread(&base_type::run, this->shared_from_this()).detach();
#endif
    }
  };
  ...
  struct run_it {
    csbl::shared_ptr<boost::detail::shared_state_base> that_;

    run_it(csbl::shared_ptr<boost::detail::shared_state_base> that) : that_ (that) {}
...
namespace detail {
  ...
  public:
    ...
    {
      this->set_executor_policy(executor_ptr_type(new executor_ref<Ex>(ex)));
      this->base_type::init(lk);
    }
    void init(boost::unique_lock<boost::mutex> &lk, executor_ptr_type& ex)
    {
      this->set_executor_policy(ex);
      this->base_type::init(lk);
    }
    void launch_continuation() {
  ...
  make_future_deferred_continuation_shared_state(
    ...
    typedef typename decay<Fp>::type Cont;
    csbl::shared_ptr<future_deferred_continuation_shared_state<F, Rp, Cont> >
  ...
  make_future_async_continuation_shared_state(
    ...
    typedef typename decay<Fp>::type Cont;
    csbl::shared_ptr<future_async_continuation_shared_state<F,Rp, Cont> >
  ...
  template<typename F, typename Rp, typename Fp>
  BOOST_THREAD_FUTURE<Rp>
  make_future_executor_continuation_shared_state(executor_ptr_type& ex,
      boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    csbl::shared_ptr<future_executor_continuation_shared_state<F,Rp, Cont> >
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_executor_continuation_shared_state_ex(Ex& ex,
      boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    executor_ptr_type sex(new executor_ref<Ex>(ex));
    return make_future_executor_continuation_shared_state<F, Rp, Cont>(
        sex, lock, boost::move(f), boost::forward<Fp>(c));
  }
#endif
  ...
  make_shared_future_deferred_continuation_shared_state(
    ...
    typedef typename decay<Fp>::type Cont;
    csbl::shared_ptr<shared_future_deferred_continuation_shared_state<F, Rp, Cont> >
  ...
  make_shared_future_async_continuation_shared_state(
    ...
    typedef typename decay<Fp>::type Cont;
    csbl::shared_ptr<shared_future_async_continuation_shared_state<F,Rp, Cont> >
  ...
  template<typename F, typename Rp, typename Fp>
  BOOST_THREAD_FUTURE<Rp>
  make_shared_future_executor_continuation_shared_state(executor_ptr_type& ex,
      boost::unique_lock<boost::mutex> &lock, F f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    csbl::shared_ptr<shared_future_executor_continuation_shared_state<F, Rp, Cont> >
  ...
  template<typename Ex, typename F, typename Rp, typename Fp>
  BOOST_THREAD_FUTURE<Rp>
  make_shared_future_executor_continuation_shared_state_ex(Ex& ex,
      boost::unique_lock<boost::mutex> &lock, F f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    executor_ptr_type sex(new executor_ref<Ex>(ex));
    return make_shared_future_executor_continuation_shared_state<F, Rp, Cont>(
        sex, lock, f, boost::forward<Fp>(c));
  }
#endif
}
  ...
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      executor_ptr_type ex = this->future_->get_executor();
      return BOOST_THREAD_MAKE_RV_REF((boost::detail::make_future_executor_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(ex,
                    lock, boost::move(*this), boost::forward<F>(func)
                )));
#endif
    } else {
  ...
  BOOST_THREAD_FUTURE<R>::then(Ex& ex, BOOST_THREAD_FWD_REF(F) func) {
    ...
    boost::unique_lock<boost::mutex> lock(this->future_->mutex);
    executor_ptr_type h(new executor_ref<Ex>(ex));
    return BOOST_THREAD_MAKE_RV_REF((boost::detail::make_future_executor_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(h,
                  lock, boost::move(*this), boost::forward<F>(func)
              )));
  }
#endif
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)  {
    ...
    typedef typename decay<F>::type Cont;
    typedef typename boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
  ...
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)  const
  {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      executor_ptr_type ex = this->future_->get_executor();
      return BOOST_THREAD_MAKE_RV_REF((boost::detail::make_shared_future_executor_continuation_shared_state<shared_future<R>, future_type>(ex,
                    lock, *this, boost::forward<F>(func)
                )));
#endif
    } else {
  ...
  shared_future<R>::then(Ex& ex, BOOST_THREAD_FWD_REF(F) func)  const
  {
    ...
    boost::unique_lock<boost::mutex> lock(this->future_->mutex);
    executor_ptr_type h(new executor_ref<Ex>(ex));
    return BOOST_THREAD_MAKE_RV_REF((boost::detail::make_shared_future_executor_continuation_shared_state<shared_future<R>, future_type>(h,
                  lock, *this, boost::forward<F>(func)
              )));
  }
#endif
...
namespace detail
{
  ...
  ////////////////////////////////
  template<typename F>
  struct future_when_all_vector_shared_state: shared_state<csbl::vector<F> >
  {
  ...
  ////////////////////////////////
  template<typename F>
  struct future_when_any_vector_shared_state: shared_state<csbl::vector<F> >
  {
    typedef csbl::vector<F> vector_type;
    typedef typename F::value_type value_type;
    vector_type vec_;
  ...
  struct future_when_all_tuple_shared_state: shared_state<Tuple>
  {
    Tuple tup_;
  ...
  public:
    ...
    future_when_all_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Fs) ... futures) :
      tup_(csbl::make_tuple(boost::forward<F>(f), boost::forward<Fs>(futures)...))
    {
    }
  ...
  template< typename Tuple, typename T0, typename ...T >
  struct future_when_any_tuple_shared_state: shared_state<Tuple>
  {
    Tuple tup_;
    typedef typename make_tuple_indices<1+sizeof...(T)>::type Index;
